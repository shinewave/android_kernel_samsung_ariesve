...
struct wakeup_source imei_wake_lock;
struct wakeup_source dpram_wake_lock;
struct wakeup_source silent_wake_lock;
...
#define CLUSTER_SEGMENT 1550
	...
	if (size > CLUSTER_SEGMENT){
		while (size) {
			...
			tty_insert_flip_string(device->serial.tty->port, psrc + retval, copied_size);
	...
	return tty_insert_flip_string(device->serial.tty->port, psrc, size);
}
				...
				unsigned long arg)
{
	...
	switch (cmd) {
		...
		case HN_DPRAM_WAKELOCK:
			__pm_stay_awake(&imei_wake_lock);
			return 0;

		case HN_DPRAM_WAKEUNLOCK:
			__pm_relax(&imei_wake_lock);
			return 0;
...
static void cmd_error_display_handler(void)
{
	...
	__pm_stay_awake(&silent_wake_lock);
...
static void non_command_handler(u16 non_cmd)
{
	...
	if (non_cmd & INT_MASK_SEND_F) {
		__pm_wakeup_event(&dpram_wake_lock, HZ/2);
	...
	if (non_cmd & INT_MASK_SEND_R) {
		__pm_wakeup_event(&dpram_wake_lock, 6*HZ);
...
static ssize_t silent_read_proc_debug(struct file *filp, char __user * buffer,
...
static ssize_t silent_write_proc_debug(struct file *filp, const char __user *buffer,
		                    size_t count, loff_t *offset)
{
	char *buf;
...
static int dump_read_proc_debug(struct file *filp, char __user * buffer,
								size_t count, loff_t *offset)
{
	char *buf;
	int ret;
...
static int dump_write_proc_debug(struct file *file, const char __user *buffer,
		                    size_t count, loff_t *offset)
{
	char *buf;
...
static int nosim_proc_read(struct file *filp, char __user * buffer,
							size_t count, loff_t *offset)
{
	return 0;
}

static int nosim_proc_write(struct file *file, const char __user *buffer,
		                    size_t count, loff_t *offset)
{
	char *buf;
...
static int __init dpram_init(void)
{
	...
	wakeup_source_init(&imei_wake_lock, "IEMI");
	wakeup_source_init(&dpram_wake_lock, "DPRAM");
	wakeup_source_init(&silent_wake_lock, "SILENT_RESET");
	platform_device_register_simple("dpram", -1, NULL, 0);

	// For silent ram dump mode
	
	ent = proc_create("silent", S_IRWXUGO, NULL, &proc_file_ops);
	smem_flag = (struct smem_info *) smem_alloc(SMEM_ID_VENDOR2, sizeof(struct smem_info));
	if(smem_flag->info == 0xAEAEAEAE)
		silent_value = 1;

	proc_file_ops.read = dump_read_proc_debug;
	proc_file_ops.write = dump_write_proc_debug;
	ent = proc_create("dump_enable", S_IRWXUGO, NULL, &proc_file_ops);
	smem_flag->info = 0xAEAEAEAE;
	printk("[Silent Value] : %d\n", silent_value);

	proc_file_ops.read = nosim_proc_read;
	proc_file_ops.write = nosim_proc_write;
	ent = proc_create("nosim_handler", S_IRWXUGO, NULL, &proc_file_ops);
...
{
	wakeup_source_trash(&dpram_wake_lock);
	wakeup_source_trash(&imei_wake_lock);
	wakeup_source_trash(&silent_wake_lock);
