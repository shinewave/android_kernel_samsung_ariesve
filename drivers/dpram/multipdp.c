...
struct wakeup_source pdp_wakeup_source;
...
static void vnet_defer_xmit(struct work_struct *data)
{
   ...
   struct pdp_info *dev = (struct pdp_info *)net->ml_priv;
	...
	if (ret < 0) {
		...
		netdev_get_tx_queue(net, 0)->trans_start = jiffies;
...
static int vnet_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	dev->vn_dev.xmit_task.func = vnet_defer_xmit;
...
static struct net_device *vnet_add_dev(void *priv)
{
	...
	dev = alloc_netdev(0, "pdp%d", NET_NAME_PREDICTABLE, vnet_setup);
	if (dev == NULL) {
...
#define CLUSTER_SEGMENT 1550
	...
	if (size > CLUSTER_SEGMENT){
		while (size) {
			...
			tty_insert_flip_string(tty->port, psrc + retval, copied_size);
	...
	return tty_insert_flip_string(tty->port, psrc, size);
}
...
static int multipdp_vs_read(struct pdp_info *dev, char *buf, size_t len)
{
	...
	if(len > MAX_RX_BUFF_LEN) {
		...
		if (ret > 0 && dev->vs_dev.tty != NULL) {
			...
			if( ret > 0 )
				tty_flip_buffer_push(dev->vs_dev.tty->port);
		}
		...
		if (dev->id == LOOP_BACK_CHANNEL) {
			...
			tty_insert_flip_string(dev->vs_dev.tty->port, pdp_rx_buf, ret);
			tty_flip_buffer_push(dev->vs_dev.tty->port);
		}
#else
		if(dev->vs_dev.tty == NULL)
	...
	#else			
			ret = tty_insert_flip_string(dev->vs_dev.tty->port, pdp_rx_buf, ret);
	#endif
			if( ret > 0 )
				tty_flip_buffer_push(dev->vs_dev.tty->port);
		}
		...
		if (len > MAX_PDP_DATA_LEN) {	// RF cal data?
            ...
            if ((dev->id == 1 && !fp_vsCSD) || (dev->id == 5 && !fp_vsGPS) || (dev->id == 8 && !fp_vsEFS)|| (dev->id == 25 && !fp_vsSMD)){
                ...
                while (size) {
        			...
        			if (size > 0 && dev->vs_dev.tty != NULL) 
        				insert_size = tty_insert_flip_string(dev->vs_dev.tty->port,
										prx_buf+retval, copied_size);
            ...
            if(retval > 0){
                ...
                if((dev->id == 1 && !fp_vsCSD) || (dev->id == 5 && !fp_vsGPS) || (dev->id == 8 && !fp_vsEFS)|| (dev->id == 25 && !fp_vsSMD)) {
        			...
        			insert_size = tty_insert_flip_string(dev->vs_dev.tty->port, pdp_rx_buf, retval);
...
//		__pm_wakeup_event(&pdp_wakeup_source, 12*HZ/2);
		__pm_wakeup_event(&pdp_wakeup_source, pdp_wake_time);
#ifdef	NO_TTY_DPRAM
...
static int multipdp_proc_read(struct file* filp, char __user *buffer,
...
{
	int ret;
	struct task_struct *dpramThread;
#ifdef CONFIG_ENABLE_TTY_CIQ
	...
	dpramThread = kthread_run(dpram_thread, NULL, "dpram_thread");
	if (IS_ERR_OR_NULL(dpramThread)) {
		EPRINTK("kthread_run() failed when starting dpram thread\n");
		return;
	}
...
static int __init multipdp_init(void)
{
	...
	const struct file_operations proc_file_ops = {
		.read = multipdp_proc_read,
	};

	DPRINTK(2, "++\n");

	wakeup_source_init(&pdp_wakeup_source, "MULTI_PDP");
...
#ifdef CONFIG_PROC_FS
	proc_create(APP_DEVNAME, S_IRUGO, NULL, &proc_file_ops);
#endif
...
{
	wakeup_source_trash(&pdp_wakeup_source);
#ifdef CONFIG_PROC_FS
