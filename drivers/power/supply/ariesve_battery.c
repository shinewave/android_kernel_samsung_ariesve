...
#include <linux/pmic8058/pmic8058.h>
...
static DEVICE_ATTR(jig , S_IRUGO | S_IWUSR, jig_show, NULL);
...
static const struct power_supply_desc msm_psy_ac_desc = {
	.name = "ac",
...
static const struct power_supply_desc msm_psy_usb_desc = {
	.name = "usb",
...
static const struct power_supply_desc msm_psy_batt_desc = {
	.name = "battery",
...
static void fg_set_alert_ext(unsigned long arg)
{
	...
	if (msm_batt_info.charging_source == NO_CHG)
	{
		...
		is_alert = 1;
		pm_wakeup_event(&msm_batt_info.msm_psy_batt->dev, 30 * TIME_UNIT_SECOND);//FIXME
	}
	else
		is_alert = 0;
}
...
static void msm_batt_cable_status_update(void)
{
	...
	if (charger_type != CHARGER_TYPE_NONE)	// USB, TA, Wireless
	{
		...
		pm_stay_awake(&msm_batt_info.msm_psy_batt->dev);
	}
	else	// No charger
	{
		...
		msm_batt_info.batt_status = POWER_SUPPLY_STATUS_DISCHARGING;
		msm_batt_chg_en(STOP_CHARGING);
		pm_wakeup_event(&msm_batt_info.msm_psy_batt->dev, 5 * TIME_UNIT_SECOND);//FIXME
	}
...
static int msm_batt_cleanup(void)
{
	...
	del_timer_sync(&msm_batt_info.timer);
	msm_batt_remove_attrs(&msm_batt_info.msm_psy_batt->dev);
...
static int msm_batt_probe(struct platform_device *pdev)
{
	...
	struct power_supply_config pwr_sply_conf = {
	...
	if (pdata->avail_chg_sources & AC_CHG) {
		supply = power_supply_register(&pdev->dev, &msm_psy_ac_desc, &pwr_sply_conf);
		if (!supply) {
			dev_err(&pdev->dev,
		...
		msm_batt_info.msm_psy_ac = supply;
		msm_batt_info.avail_chg_sources |= AC_CHG;
	}

	if (pdata->avail_chg_sources & USB_CHG) {
		supply = power_supply_register(&pdev->dev, &msm_psy_usb_desc, &pwr_sply_conf);
		if (!supply) {
			dev_err(&pdev->dev,
				"%s: power_supply_register failed\n", __func__);
			msm_batt_cleanup();
		...
		msm_batt_info.msm_psy_usb = supply;
	...
	supply = power_supply_register(&pdev->dev, &msm_psy_batt_desc, NULL);
	if (!supply) {
		dev_err(&pdev->dev, "%s: power_supply_register failed\n", __func__);
		msm_batt_cleanup();
		return -1;
	}
	msm_batt_info.msm_psy_batt = supply;
	...
	msm_batt_create_attrs(&msm_batt_info.msm_psy_batt->dev);
...
static void msm_batt_shutdown(struct device *dev)
{
	del_timer_sync(&msm_batt_info.timer);
}
