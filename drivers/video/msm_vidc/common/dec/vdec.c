					...
					struct vdec_meta_buffers *meta_buffers)
{
	...
	size_t buffer_size = 0, buffer_size_iommu = 0;
	ion_phys_addr_t iova = 0, iova_iommu = 0;
	...
	if (!vcd_get_ion_status()) {
		if (get_pmem_file(vcd_meta_buffer->pmem_fd,
				(phys_addr_t*)&vcd_meta_buffer->physical_addr,
		...
		if (get_pmem_file(vcd_meta_buffer->pmem_fd_iommu,
				(phys_addr_t *) &vcd_meta_buffer->physical_addr_iommu,
				(unsigned long *) (&vcd_meta_buffer->
		...
		client_ctx->meta_buffer_ion_handle = ion_import_dma_buf_fd(
		...
		client_ctx->meta_buffer_iommu_ion_handle = ion_import_dma_buf_fd(
					client_ctx->user_ion_client,
					...
					struct vdec_h264_mv *mv_data)
{
	...
	struct msm_mapped_buffer *mapped_buffer = NULL;
	u32 vcd_status = VCD_ERR_FAIL, flags = 0;
	size_t len = 0;
	struct file *file;
	int rc = 0;
	...
	if (!vcd_get_ion_status()) {
		if (get_pmem_file(vcd_h264_mv_buffer->pmem_fd,
			(phys_addr_t*) &vcd_h264_mv_buffer->physical_addr,
		...
		flags = MSM_SUBSYSTEM_MAP_IOVA;
		mapped_buffer = msm_subsystem_map_buffer(
			(phys_addr_t) vcd_h264_mv_buffer->physical_addr, len,
		...
		client_ctx->h264_mv_ion_handle = ion_import_dma_buf_fd(
					client_ctx->user_ion_client,
		...
		if (res_trk_check_for_sec_session() ||
		   (res_trk_get_core_type() == (u32)VCD_CORE_720P)) {
			rc = ion_phys(client_ctx->user_ion_client,
				client_ctx->h264_mv_ion_handle,
				(ion_phys_addr_t *) (&(vcd_h264_mv_buffer->
				physical_addr)), &len);
			if (rc) {
				ERR("%s():get_ION_kernel physical addr fail\n",
					__func__);
			...
			if (rc || !iova) {
				ERR(
				"%s():get_ION_kernel physical addr fail, rc = %d iova = 0x%x\n",
					__func__, rc, iova);
				goto ion_map_error;
			}
			 ...
			 unsigned cmd, unsigned long u_arg)
{
	...
	u32 vcd_status;
	unsigned long kernel_vaddr, ker_vaddr;
	...
	case VDEC_IOCTL_SET_SEQUENCE_HEADER:
	{
		...
		if (!vcd_get_ion_status()) {
			if (get_pmem_file(seq_header.pmem_fd, &phy_addr,
					&kernel_vaddr, (unsigned long*) &len, &pmem_file)) {
				ERR("%s(): get_pmem_file failed\n", __func__);
				return false;
			}
			put_pmem_file(pmem_file);
		} else {
			client_ctx->seq_hdr_ion_handle = ion_import_dma_buf_fd(
				client_ctx->user_ion_client,
				seq_header.pmem_fd);
