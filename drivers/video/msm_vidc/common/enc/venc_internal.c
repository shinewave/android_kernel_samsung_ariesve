		...
		struct venc_recon_addr *venc_recon)
{
	...
	if (!vcd_get_ion_status()) {
		if (get_pmem_file(control->pmem_fd, (phys_addr_t *)
			...
			(phys_addr_t)control->physical_addr, len,
		...
		client_ctx->recon_buffer_ion_handle[i] = ion_import_dma_buf_fd(
		...
		if (res_trk_check_for_sec_session() ||
		   (res_trk_get_core_type() == (u32)VCD_CORE_720P)) {
			...
			if (rc || !iova) {
				ERR(
				"%s():ION map iommu addr fail, rc = %d, iova = 0x%x\n",
					__func__, rc, iova);
				goto map_ion_error;
			}
