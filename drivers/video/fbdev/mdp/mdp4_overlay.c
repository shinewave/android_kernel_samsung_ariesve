...
void mdp4_overlay_iommu_pipe_free(int ndx, int all)
{
	...
	if (pipe->flags & MDP_MEMORY_ID_TYPE_FB) {
		...
		fdput(pipe->srcp0_file);
		fdput(pipe->srcp1_file);
		fdput(pipe->srcp2_file);
	...
	struct ion_handle **srcp_ihdl)
{
	...
	*srcp_ihdl = ion_import_dma_buf_fd(display_iclient, mem_id);
	...
	pr_debug("%s: ndx=%d plane=%d prev=0x%p cur=0x%p start=0x%x len=%x\n",
...
static int get_img(struct msmfb_data *img, struct fb_info *info,
	struct mdp4_overlay_pipe *pipe, unsigned int plane,
	ion_phys_addr_t *start, size_t *len, struct fd *srcp_file,
	struct ion_handle **srcp_ihdl)
{
	struct fd f;
	int ret = 0, fb_num;
	...
	if (img->flags & MDP_MEMORY_ID_TYPE_FB) {
		...
		if (MAJOR(file_inode(f.file)->i_rdev) == FB_MAJOR) {
			fb_num = MINOR(file_inode(f.file)->i_rdev);
...
int mdp4_overlay_play(struct fb_info *info, struct msmfb_overlay_data *req)
{
	...
	img = &req->data;
	get_img(img, info, pipe, 0, &start, &len, &pipe->srcp0_file, &srcp0_ihdl);
	if (len == 0) {
		pr_err("%s: pmem Error\n", __func__);
		ret = -1;
		goto end;
