...
u32 mdp4_allocate_writeback_buf(struct msm_fb_data_type *mfd, u32 mix_num)
{
	...
	if (!IS_ERR_OR_NULL(mfd->iclient)) {
		...
		pr_warn("No ION allocation for mdp!");
		addr = 0;
	...
	if (!IS_ERR_OR_NULL(mfd->iclient)) {
		...
		if (buf->write_addr) {
			...
			pr_debug("%s:%d free writeback pmem\n", __func__,
				__LINE__);*/
			pr_warn("Cannot free non-ION memory for mdp writeback! MEMORY LEAK!!");
		}
	}
	buf->write_addr = 0;
	buf->read_addr = 0;
...
static int mdp4_update_pcc_regs(uint32_t offset_bytes,
				struct mdp_pcc_cfg_data *cfg_ptr)
{
	int ret = -1;
	uint8_t* offset = (uint8_t*)offset_bytes;
...
static int mdp4_read_pcc_regs(uint32_t offset_bytes,
				struct mdp_pcc_cfg_data *cfg_ptr)
{
	int ret = -1;
	uint8_t* offset = (uint8_t*)offset_bytes;
...
int mdp4_pcc_cfg(struct mdp_pcc_cfg_data *cfg_ptr)
{
	...
	if (0x8 & cfg_ptr->ops)
		outpdw((uint8_t*)mdp_dma_op_mode,
			((inpdw((uint8_t*)mdp_dma_op_mode) & ~(0x1<<10)) |
						((0x8 & cfg_ptr->ops)<<10)));

	outpdw((uint8_t*)mdp_cfg_offset,
			((inpdw((uint8_t*)mdp_cfg_offset) & ~(0x1<<29)) |
						((cfg_ptr->ops & 0x1)<<29)));
...
int mdp4_qseed_access_cfg(struct mdp_qseed_cfg *config, uint32_t base)
{
	...
	if (config->ops & MDP_PP_OPS_WRITE) {
		...
		for (i = 0; i < config->len; i++) {
			if (!(base & 0x3FF))
				wmb();
			MDP_OUTP((uint8_t*)base , values[i]);
