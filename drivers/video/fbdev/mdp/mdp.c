...
static int mdp_histogram_enable(struct mdp_hist_mgmt *mgmt)
{
	...
	mdp_clk_ctrl(1);
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	base = (uint8_t*) (MDP_BASE + mgmt->base);
...
#ifdef CONFIG_FB_MSM_MDP40
	ret = request_irq(mdp_irq, mdp4_isr, 0, "MDP", 0);
#else
	ret = request_irq(mdp_irq, mdp_isr, 0, "MDP", 0);
#endif
	if (ret) {
