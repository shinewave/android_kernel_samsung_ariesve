...
msm_i2c_probe(struct platform_device *pdev)
{
	...
	if (pdata->rmutex) {
		remote_spinlock_id_t rmid = pdata->rsl_id;
		if (remote_spin_lock_init(&dev->r_lock, rmid) != 0)
			pdata->rmutex = 0;
	}
