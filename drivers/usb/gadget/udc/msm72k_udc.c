...
static void handle_setup(struct usb_info *ui)
{
	...
	if ((ctl.bRequestType & (USB_DIR_IN | USB_TYPE_MASK)) ==
		...
		if (ctl.bRequest == USB_REQ_GET_STATUS) {
			...
			{
				if (ctl.wIndex == OTG_STS_SELECTOR) {
#ifdef CONFIG_USB_OTG
					spin_lock_irqsave(&ui->lock, flags);
					hnp = ui->gadget.host_request_flag;
					ui->hnp_avail = 1;
					...
					u16 temp = (atomic_read(&ui->self_powered)
...
static void usb_prepare(struct usb_info *ui)
{
	...
	usb_ep_set_maxpacket_limit(&ui->ep0in.ep, 64);
	usb_ep_set_maxpacket_limit(&ui->ep0out.ep, 64);
...
msm72k_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
	...
	usb_ep_set_maxpacket_limit(_ep, le16_to_cpu(desc->wMaxPacketSize));
	config_ept(ept);
	ept->wedged = 0;
...
static int msm72k_gadget_start(struct usb_gadget *gadget,
		struct usb_gadget_driver *driver);
static int msm72k_gadget_stop(struct usb_gadget *gadget);
...
static const struct usb_gadget_ops msm72k_ops = {
	...
	.udc_start		= msm72k_gadget_start,
	.udc_stop		= msm72k_gadget_stop
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&ui->lock, flags);
	if (ui->hnp_avail)
		ui->gadget.host_request_flag = !!val;
	spin_unlock_irqrestore(&ui->lock, flags);
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	ui->pdev = pdev;
	ui->pdata = dev_get_platdata(&pdev->dev);
	...
	wakeup_source_init(&ui->wlock, "usb_bus_active");
			    ...
			    struct usb_gadget_driver *driver)
{
	...
	ui->driver = driver;
	gadget->name = driver_name;
	INIT_LIST_HEAD(&gadget->ep_list);
	gadget->ep0 = &ui->ep0in.ep;
	...
	for (n = 1; n < 16; n++) {
		struct msm_endpoint *ept = ui->ept + n;
		list_add_tail(&ept->ep.ep_list, &gadget->ep_list);
		usb_ep_set_maxpacket_limit(&ept->ep, 512);
	}
	for (n = 17; n < 32; n++) {
		struct msm_endpoint *ept = ui->ept + n;
		list_add_tail(&ept->ep.ep_list, &gadget->ep_list);
	...
	retval = usb_add_gadget_udc(&ui->pdev->dev, gadget);
	if (retval)
		goto fail;

	retval = device_create_file(&gadget->dev, &dev_attr_wakeup);
	if (retval != 0)
		dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
			"(wakeup) error: (%d)\n", retval);
	retval = device_create_file(&gadget->dev, &dev_attr_usb_state);
	if (retval != 0)
		dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
			" (usb_state) error: (%d)\n", retval);

	retval = device_create_file(&gadget->dev, &dev_attr_usb_speed);
	if (retval != 0)
		dev_err(&ui->pdev->dev, "failed to create sysfs entry:"
			" (usb_speed) error: (%d)\n", retval);

	retval = device_create_file(&gadget->dev, &dev_attr_chg_type);
	if (retval != 0)
		dev_err(&ui->pdev->dev,
			"failed to create sysfs entry(chg_type): err:(%d)\n",
					retval);
	retval = device_create_file(&gadget->dev, &dev_attr_chg_current);
	if (retval != 0)
		dev_err(&ui->pdev->dev,
...
static int msm72k_gadget_stop(struct usb_gadget *gadget)
{
	...
	msm72k_pullup_internal(gadget, 0);
	if (dev->irq) {
	...
	device_remove_file(&gadget->dev, &dev_attr_wakeup);
	device_remove_file(&gadget->dev, &dev_attr_usb_state);
	device_remove_file(&gadget->dev, &dev_attr_usb_speed);
	device_remove_file(&gadget->dev, &dev_attr_chg_type);
	device_remove_file(&gadget->dev, &dev_attr_chg_current);
	dev->driver = NULL;
