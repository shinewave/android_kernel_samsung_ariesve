...
static void msm_hsusb_request_host(void *handle, int request)
{
	...
	case REQUEST_STOP:
		...
		msm_xusb_disable_clks(mhcd);
		__pm_wakeup_event(&mhcd->wlock, HZ/2);
...
static int ehci_msm_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(&mhcd->wlock, dev_name(&pdev->dev));
	...
	if (retval < 0) {
		wakeup_source_trash(&mhcd->wlock);
		usb_put_hcd(hcd);
