...
{
	struct msm_otg *dev = container_of(otg->usb_phy, struct msm_otg, phy);
	enum usb_otg_state state;
	unsigned long flags;

	spin_lock_irqsave(&dev->lock, flags);
	state = dev->phy.otg->state;
	spin_unlock_irqrestore(&dev->lock, flags);
...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	while (!is_phy_clk_disabled()) {
		if (time_after(jiffies, timeout)) {
			 ...
			 * phy suspend failure*/
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_UNDEFINED;
			spin_unlock_irqrestore(&dev->lock, flags);
			queue_work(dev->wq, &dev->sm_work);
			goto out;
		}
...
static int msm_otg_set_suspend(struct usb_phy *xceiv, int suspend)
{
	...
	if (suspend) {
		...
		case OTG_STATE_A_SUSPEND:
			...
			set_bit(A_BUS_REQ, &dev->inputs);
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_A_HOST;
			spin_unlock_irqrestore(&dev->lock, flags);
			if (test_bit(ID_A, &dev->inputs) &&
...
static void msm_otg_set_id_state(int id)
{
	...
	spin_lock_irqsave(&dev->lock, flags);
	if (dev->phy.otg->state != OTG_STATE_UNDEFINED) {
		__pm_stay_awake(&dev->wlock);
		queue_work(dev->wq, &dev->sm_work);
	}
	spin_unlock_irqrestore(&dev->lock, flags);
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		if (atomic_read(&dev->chg_type) ==
				USB_CHG_TYPE__WALLCHARGER) {
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_B_PERIPHERAL;
			spin_unlock_irqrestore(&dev->lock, flags);
			work = 1;
			break;
		}
		...
		if ((test_bit(ID, &dev->inputs)) &&
				!test_bit(ID_A, &dev->inputs)) {
			dev->phy.otg->state = OTG_STATE_B_IDLE;
		} else {
			set_bit(A_BUS_REQ, &dev->inputs);
			dev->phy.otg->state = OTG_STATE_A_IDLE;
		}
		spin_unlock_irqrestore(&dev->lock, flags);
	...
	case OTG_STATE_B_IDLE:
		...
		if (!test_bit(ID, &dev->inputs) ||
			...
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_B_SRP_INIT;
			spin_unlock_irqrestore(&dev->lock, flags);
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &dev->inputs) ||
			...
			msm_otg_start_peripheral(dev->phy.otg, 0);
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_B_WAIT_ACON;
			spin_unlock_irqrestore(&dev->lock, flags);
	...
	case OTG_STATE_B_WAIT_ACON:
		if (!test_bit(ID, &dev->inputs) ||
			...
			clear_bit(A_BUS_SUSPEND, &dev->inputs);
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_B_HOST;
			spin_unlock_irqrestore(&dev->lock, flags);
			if (test_bit(ID_C, &dev->inputs)) {
				atomic_set(&dev->chg_type, USB_CHG_TYPE__SDP);
				msm_otg_set_power(&dev->phy, USB_IDCHG_MAX);
	...
	case OTG_STATE_A_IDLE:
		...
		if (test_bit(ID, &dev->inputs) &&
			...
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_A_WAIT_VRISE;
			spin_unlock_irqrestore(&dev->lock, flags);
	...
	case OTG_STATE_A_WAIT_BCON:
		if ((test_bit(ID, &dev->inputs) &&
			...
			msm_otg_start_host(dev->phy.otg, REQUEST_STOP);
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_A_VBUS_ERR;
			spin_unlock_irqrestore(&dev->lock, flags);
			/* Reset both phy and link */
			otg_reset(&dev->phy, 1);
	...
	case OTG_STATE_A_HOST:
		if ((test_bit(ID, &dev->inputs) &&
			...
			pr_debug("!a_bus_req\n");
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_A_SUSPEND;
			spin_unlock_irqrestore(&dev->lock, flags);
			if (dev->phy.otg->host->b_hnp_enable) {
	...
	case OTG_STATE_A_SUSPEND:
		if ((test_bit(ID, &dev->inputs) &&
			...
			msm_otg_del_timer(dev);
			spin_lock_irqsave(&dev->lock, flags);
			dev->phy.otg->state = OTG_STATE_A_PERIPHERAL;
			spin_unlock_irqrestore(&dev->lock, flags);
				...
				size_t count, loff_t *ppos)
{
	...
	temp += scnprintf(buf + temp, OTG_INFO_SIZE - temp,
			...
			state_string(dev->phy.otg->state),
			dev->pdata->otg_mode,
			dev->inputs,
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(&dev->wlock, "msm_otg");

	dev->wq = alloc_workqueue("k_otg", 0, 0);
	if (!dev->wq) {
		ret = -ENOMEM;
		goto free_wlock;
	}
	...
	usb_add_phy(&dev->phy, USB_PHY_TYPE_USB2);
#ifdef CONFIG_USB_MSM_ACA
