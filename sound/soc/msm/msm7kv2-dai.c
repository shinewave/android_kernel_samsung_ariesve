...
static const struct snd_soc_component_driver msm_pcm_component_driver = {
	.name = "msm-pcm",
};
...
{
	dev_info(&pdev->dev, "%s: dev name %s\n", __func__, dev_name(&pdev->dev));
	return snd_soc_register_component(&pdev->dev, &msm_pcm_component_driver,
						msm_pcm_cpu_dais, ARRAY_SIZE(msm_pcm_cpu_dais));
}
